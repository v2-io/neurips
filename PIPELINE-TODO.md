...
**Status:** NOT-REPRODUCING-IN-CURRENT-CODE (2026-05-06). Tested the exact pre-revert pattern from 03-llm-hallucinate (`"$\mathbb{E}\,d \leq C\sqrt{I}$"` from src/05-track2-fisher-rao.md:11) plus 10 stress-test variants (start/end of paragraph, parenthesized, with punctuation, multi-math inside the quote pair, single-quoted outer, etc.). All convert correctly to LaTeX `` `` `` `` `` `` or curly quotes via the NeurIPS parser subclass. Edge cases that DO break (no-whitespace-mid-word `prose"$y$"prose`, double-double `""$x$""`) are pathological and unlikely to appear in real prose. Possible explanations: (a) the bug existed in an earlier kramdown release and got resolved by an interim update; (b) the agent saw a different render artifact and attributed it to smart-quote; (c) some interaction with another bug in their specific source that's no longer triggering. Per-paper agents can revert their Unicode-curly-quote workaround back to ASCII `"` and the build will smart-quote correctly. If the bug DOES re-surface with a new reproducer, please update this entry with the exact triggering line — we'll then fix at parser-subclass level.
