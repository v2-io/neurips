...
- [x] **A4. NeurIPS paper checklist — resolution recorded 2026-05-06.** Original framing posed a binary (a) shared `common/checklist.tex` via manifest row vs (b) segment-per-question. Both are wrong for the actual situation — the checklist needs *filled-in answers per paper* (theory papers vs experimental papers answer differently; reviewers expect populated form, not blank questions), so per-paper ownership is the only viable model. **De-facto resolution (option c):** per-paper agent owns `<paper>/src/checklist.tex`, copied from `common/checklist.tex` template, fills in answers; manifest gets a `Checklist`-typed row pointing at it. Build pipeline already supports this end-to-end (proven on 00-test-paper and 02-unified-convergence-rl). AUTHORING §1.12 already documents the per-paper-author flow. **Pipeline-side residuals:** none load-bearing. The 00-test-paper 6-item stub is harmless (manifest mechanic is what 00 demonstrates, not checklist content). 01 and 03 received `CHECKLIST.md` recipe drops + TODO entries 2026-05-06; per-paper-agent work from here. Optional polish — build-time warning if a submission manifest lacks a Checklist row — discretionary, not queued.
...
- [x] **E3. Page-budget tool** — `bin/page-budget` (Ruby port of old workspace's `bin/page-budget`). Parses `<paper-dir>/.build/<manifest>/<manifest>.aux` for `\@writefile{toc}{\contentsline ...}` entries; finds first appendix/references marker; reports main-text page count vs 9-pp limit + per-section progression. Usage: `bin/page-budget [<paper-dir>...] [--manifest STEM]`. Default manifest is `neurips-2026-paper`. Critical during trim work; per-paper agents can run repeatedly after each cut to track progress. **Bibliography-region sharpening (T1.5) landed `9160dce`:** when tier-1 (first-appendix marker) is the explicit signal AND `\bibcite` count > 0, subtract estimated bib pages from the appendix-start to recover the true main-text-end. Surfaces as marker `appendix-minus-bib(Npp est. for K refs)` in the report. Resolves the 1-2pp overcount that was visible across all three real papers (each subtracts ~2pp now). **Build-time integration landed `17cb12e`:** `bin/build` runs `page-budget` automatically on each successful build and prints stats inline.
...
- [x] **F6. Schema separation: `note:` (scholarly) vs `internal_note:` (agent-meta).** Surfaced 2026-05-06 across all three NeurIPS 2026 papers: many YAML entries had agent-meta in their `note:` field ("Cited in 03-llm-hallucinate §1 / §7.1 for VC-dim impossibility", "Year-of-record decision deferred per source OUTLINE", "Verified 2026-05-05 via OpenReview ID..."), which got emitted to BibTeX's `note = {...}` and rendered by `unsrtnat` as bibitem trailer text in the published bibliography. AUTHORING §3.9 chronicle-voice anti-pattern applies. Fix: introduce `internal_note:` YAML field for working metadata; `bin/refs emit` already only outputs the canonical bibtex-field whitelist (`bibtex_fields` in bin/refs:138) so any non-listed YAML key — including `internal_note` — is silently ignored at emit time. Documented the convention in `refs/README.md` with explicit `note:` vs `internal_note:` distinction. Per-paper agents can migrate at their pace; the migration is content territory (split mixed-content `note:` fields into scholarly + internal halves). Optional follow-up: `bin/refs lint` heuristic warning when a `note:` field text matches chronicle-voice patterns ("Cited in 0N-", "Verified 20", "flagged in TODO.md") — surface candidates for migration without enforcing.

- [ ] **F7. Migrate to `~/src/relata/` — the cross-project shared bibliography database.** Surfaced 2026-05-13. The `~/src/refs/`-pattern bib system was elevated to `~/src/relata/` (Latin "things related/narrated") as a cross-project shared database, intended to be consumed by both this NeurIPS workspace AND the agentic-systems framework AND any future projects (Synthese papers, embeddings work, future Paperpile imports). `~/src/relata/` ports the same per-entry YAML + append-only verifications + deny-list architecture from this directory; the CLI is `~/src/relata/bin/relata` (effectively the same script with paths re-rooted). It also adds two schema fields not currently here: `citation_status` (`pre-publication` / `in-review` / `preprint` / `published` / `withdrawn`) and `applicable_anonymity` (boolean + reason — distinct from self-citation; flags entries whose citation in a blind-review submission would deanonymize the submitter). Migration plan when this becomes worth doing: (a) copy `refs/entries/*.yml` and `refs/verifications/<key>/` into `~/src/relata/` (skip-on-conflict; relata may already have some via the migration agent); (b) update `bin/build` to call `~/src/relata/bin/relata emit ...` instead of the local `bin/refs emit ...`; (c) keep this `refs/` directory in place during transition (or symlink); (d) eventually retire the local `refs/` and let relata be the single source of truth. **Not yet scheduled** — current `refs/` works; migration is an organizational improvement not a fix. Worth doing when the next substantive citation work happens here OR when an agent here needs entries from relata (e.g., something added by ASF / Synthese that NeurIPS wants). See `~/src/relata/README.md` for the schema extension docs and current state.

  *Datapoint from 2026-07-29 (artifact-refresh pass).* A stray `refs.bib` was sitting untracked at `02-unified-convergence-rl/`, emitted by `bin/relata emit` on 2026-05-14 — so someone already ran a trial emission from relata against this workspace. Comparing it against the tracked `unified-rl-neurips-2026.extracted.bib`, relata's copy is *better* on the entries where they disagree: full given name (`Pearl, Judea` vs `Pearl, J`) and a properly separated publisher/edition on the Pearl book, where the local `refs/` entry has collapsed them into `publisher = {(2nd ed.). Cambridge.}`. It's a small sample (two entries), but it points the same way — the local `refs/entries/` almost certainly holds more of these collapsed-field artifacts from the original 164-entry import. Whoever picks up F7 may want to diff the two emissions across all three papers before step (a), since the direction of the copy matters: relata-wins is likely the right default on field-hygiene conflicts, not skip-on-conflict. The stray file itself is now gitignored in that submodule rather than deleted.
...
**Status:** RESOLVED-IN-`0c2a4f8`. Joseph's page-span hypothesis was on the right track — the crash root cause is hyperref's link-state machinery generating malformed startlink/endlink across page boundaries when certain combinations of `\Cref{undefined}`, `\cite{undefined}`, or even `\cite{defined}`-adjacent-to-placeholder constructs land near a page break under super-style natbib + cleveref. `breaklinks=true` doesn't tame it; `\hypersetup{draft=true}` placed in body is silently ignored ("Option `draft' has already been used"). The fix: two-stage.

1. **Defensive placeholder substitution at body-assembly time.** New `Builder#resolve_unresolved_refs` walks the assembled body, collects every `\label{X}` and parses `<paper>/refs.bib` (or umbrella `refs/entries/*.yml` as fallback) for valid cite-keys, then rewrites every `\Cref{X}` / `\eqref{X}` / `\cite[opt]{X}` (and natbib variants — `\citet`, `\citep`, `\citealt`, `\citealp`, `\citeauthor`, `\citeyear`) where X is undefined to a `[?\,label]` placeholder. The placeholder is plain text — bracketed, with a thin space — chosen empirically because `\textbf{[?~label]}` re-triggers the same hyperref nesting fatal even when the label is plain text (somehow `\textbf` interacts with hyperref's link-state tracking around adjacent valid cites). Visible in the rendered PDF; lints as a build warning.

2. **`\PassOptionsToPackage{draft}{hyperref}` when any refs are unresolved.** Step 1 alone wasn't enough — hyperref still generated link nesting bugs around the placeholders in some configurations. Hyperref draft mode disables ALL hyperlinks (cite, cref, TOC, footnote) for that build, sidestepping the link-state crash entirely. Has to be `\PassOptionsToPackage` BEFORE `\usepackage{hyperref}` (which the neurips_2026 sty does internally). Conditional on `@has_unresolved_refs`: full-paper / neurips-2026-paper builds with all refs resolved keep clickable hyperlinks unchanged.
...
### [01-tragedy + general] cleveref renders appendix-bound `\section` as "Section X" rather than "Appendix X" — flagged 2026-05-06 by 01-tragedy migration agent

**Symptom.** A cross-ref `\Cref{sec-empirical}` to a heading inside an Appendix-typed manifest segment renders as `Section H` rather than `Appendix H` in the PDF. Reader sees a "Section H" reference and has to figure out it's actually an appendix.

**Context.** In `01-tragedy-confident-agent/src/02-agent-state.md` (a main-text segment), the prose `is used in the simulation of [[#^sec-empirical]]` cross-references the `## Empirical Illustration ^sec-empirical` heading in `src/05-setup.md`. In `OUT.neurips-2026-paper.md`, the latter is typed `Appendix` (becomes App H), but cleveref produces "Section H" because the `\section` command is what the converter emits regardless of `\appendix` injection. The cleveref label-type recorded in `.aux` is `section.X`, not `appendix.X`.

This is structurally analogous to the shared-counter bug previously fixed for theorem-likes (the `aliascnt` patch in commit `185d31a` or thereabouts) — same shape, different counter pair. The fix would be to have the converter emit a different counter for headings in appendix-typed manifest segments, OR to use cleveref's `\crefname{section}` in main-flow and switch to `\crefname{appendix}` after `\appendix`, OR an `aliascnt` for `appendix` that points at the section counter while letting cleveref see the right type.
