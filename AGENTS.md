...
**Don't *triage* audit findings — *route* them.** "Triage" implies substantive judgment has been made when it hasn't. The triage frame's cadence — *judge each cleanly and queue* — collapses the mental space the strengthening attempt needs, defaulting any candidate-soften item to a soften before the strengthening attempt has happened. Routing is fine and good practice: sort findings by file affected, stage of work, or who can address them — that's organizational hygiene. The substantive judgment ("does the strengthening attempt succeed?") is a *separate* step that should happen in *spike-mode cadence*, not in the routing pass. The trigger to catch in your own writing is the word *triage* itself. When you find yourself "triaging audit findings," ask whether you're routing (organizational) or judging (substantive). If judging, you've collapsed the substantive step into the organizational one — and the substantive step is exactly where strengthening attempts go to die. The linguistic *triage* is the warning, even more reliable than the body-feel hedge-clause signal. Empirically validated: the 03-llm-hallucinate-bound A2 cycle (2026-05-07) — the parent agent triaged Codex's sharpness finding, noted *both* the strengthening route (two-point witness, given verbatim by the auditor) and the soften route, then labeled the soften "cleaner" without running the math on the strengthening. The math (when later run) was 5 minutes; the witness gave first-moment sharpness directly. The robustness of the triage frame at suppressing strengthening attempts is sharp enough that *even when the auditor hands the construction on a plate*, triage cadence routes around it.

**Escalation path when the in-context strengthening attempt comes up empty.** A short mental gesture toward strengthening that "doesn't see anything obvious" is *not* the strengthening attempt failing — it's the attempt not fitting the cadence of in-context triage. The reliable next move when this happens: launch a high-capacity sub-agent (Opus, ideally 4.7; briefing mechanics in §5.4) with a peer-voiced brief built around three legitimate spike-completion states — *succeed beyond what we already claim*, *succeed in strengthening the math to match the existing claim*, or *succeed in finding the fundamental problem-domain reason no such claim is provable*. All three leave the project better. The boundary case is as valuable as the strengthening case, since it maps structure that prevents future agents from re-attempting the same dead-end without new evidence.

Why this works: depth of attention is a resource separable from raw capability. Same model, different framing — different result. A parent in triage cadence (multiple threads, fast-judge mode) tends to give any single thread ~30 seconds of mathematical attention before defaulting to a soften; the same model under explicit *push the math hard, no time pressure, document failures diligently* permission gives 10+ minutes of focused investigation. The textbook lemma that wasn't visible under triage is often immediately available under spike conditions. The trigger to catch is: hedge clauses forming before honest math time on a candidate-soften finding — at that point, route to spike mode rather than triage-resolve. Empirically validated: 03-llm-hallucinate-bound A7 cycle (2026-05-07) — the parent agent's 30-second "strengthening fails" conclusion got overturned by a 9-minute Opus spike that found a textbook lemma plus a Class-1 reduction theorem across the full Stuart-school hypothesis space, strictly stronger than the original claim could support. Spike report at `03-llm-hallucinate-bound/spikes/A7-stuart-school-mapping/report.md`. The §3.1 principle was loaded into the parent's context and cited in the triage it was writing; what was missing was the *trigger to escalate*. Treat any candidate-soften that survived a brief in-context attempt as a route to spike mode, not as a triage-resolvable decision.

*Full reasoning + worked examples:* `~/.claude/projects/-Users-josephwecker-v2-src-agentic-systems/memory/feedback_strengthen_before_soften.md`. Triage-cadence-skips-strengthening failure mode: `~/.claude/projects/-Users-josephwecker-v2-src-neurips/memory/feedback_triage_skips_strengthening.md`.
...
3. **Frame the brief around three legitimate completion states**, not a single "find the answer" target — *succeed beyond what we currently claim*, *succeed in strengthening the math to match the existing claim*, or *succeed in finding the fundamental problem-domain reason no such claim is provable*. Each is a valid spike outcome; the third is as valuable as the first two because it maps structural boundary that prevents future agents from re-attempting dead-ends without new evidence (see §3.1 escalation-path paragraph for the failure mode this corrects). The trichotomy framing also gives the spike agent permission to *find* a stronger claim than the parent had in mind, not just to ratify the parent's framing — sometimes the right answer is "your A7 question undershoots; here's the actually-load-bearing claim."
4. **Record failures as diligently as successes.** A thoughtful attempt that fails reveals something fundamental that almost always leads to a strengthening or deeper understanding. The "why didn't it work" is the result.
5. **Tell them about local resources** — `~/src/agentic-systems/` (ASF working materials), `~/src/agentic-systems/ref/INDEX.md` (reference index), web search + paper download requests allowed.
6. **Output to `spikes/<name>/report.md`** — they can flag in-flight requests for papers / strategic calls separately at the end.
7. **Length: as needed.** No time pressure on spikes. Serve truth, not a clock.
