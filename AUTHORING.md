...
Include `src/checklist.tex` (or equivalent raw-TeX segment) via a `Checklist`-typed manifest row. The build injects `\newpage` before any segment whose Type is `Checklist`. Wiring the canonical `common/checklist.tex` into the build is `PIPELINE-TODO.md` §A4.
...
`key` is the canonical bibkey in the umbrella's `refs/entries/<key>.yml`. The build auto-emits a per-paper `.bib` from `refs/entries/` into `.build/<stem>/<stem>.references.bib` before each compile, so the umbrella database is the actual source of truth. Bib keys are canonical — no same-year ambiguity (the Hintikka-1991 problem disappears), no multi-author truncation question. Editor / IDE completion against `refs/entries/*.yml` makes lookup ergonomic; the cognitive shape matches our other key-form anchors (`[[#^thm-main]]`). To add an entry: `bin/refs add <key>` (paste BibTeX or scaffold-and-fill); never hand-edit a per-paper `.bib` file.
...
├── .build/                       # gitignored: per-manifest ephemeral build artifacts
│   └── <stem>/                   # one subdir per OUT.<stem>.md
│       ├── <stem>.tex            # rendered LaTeX
│       ├── <stem>.references.bib # auto-emitted by `bin/refs emit` before lualatex
│       ├── <stem>.pdf            # compiled PDF (canonical inside .build/)
│       └── <stem>.{aux,bbl,log,…}
├── <stem>.pdf                    # copy of the build's PDF on success — author chooses to track or gitignore
├── <stem>.prior.pdf              # last-known-good PDF (snapshot before each build); gitignore-worthy
├── <stem>.extracted.bib          # repo-visibility snapshot of the emitted bib — recommended track
└── _archive/                     # frozen artifacts (integrated audits, completed spike directories)
```

The umbrella owns: `bin/build` (pipeline), `bin/refs` (bibliography database + emit), `common/` (LaTeX template + sty), `refs/entries/*.yml` (bib source-of-truth). Per-paper repos own only their own content; pipeline behavior is shared.

**Bib edits go through `bin/refs add` / `refs/entries/<key>.yml`, never to a per-paper `.bib` file.** The build auto-emits `<stem>.references.bib` into `.build/<stem>/` before each `lualatex` run from `refs/entries/`; the `<stem>.extracted.bib` in the paper directory is a build artifact (named explicitly so) for repo-visibility of which bib content the build used. Authors who hand-edit `<stem>.extracted.bib` will see their edits silently overwritten on next build. Pre-refactor `<paper>/refs.bib` files are orphaned by this change; safe to remove.
...
10. **Build as you go.** `bin/build` from inside the submodule (or `bin/build 0N-{slug}` from the umbrella) after every meaningful chunk — not just at the end. Tight feedback loop catches issues when they're cheap to debug. Open `<stem>.pdf` visually (ephemeral build artifacts live in `.build/<stem>/`); confirm rendering, citation form, anonymization. The boundary between "I fix it" and "pause + flag":
...
- **#10** — Footnotes. Both forms work; see §2.4.
- **#11** — Inline citations. Bracketed-superscript natbib + `\cite{key}` source form; see §2.3 and `REFS-AND-CITATIONS.md`.
- **#12** — References section. Auto-rendered via `bin/refs emit` + natbib; see §1.11 and `BUILD-CHANGE.md`.
